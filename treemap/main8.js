...
}
{
  ...
  <strong style="color: #098094;">${response.length}</strong> species.`); //mention predator/prey, add depth in phase 2  + // Big N
    ...
    function zoom(d, width, height, margin, svg, nodes) {
      ...
      d3.treemap().size([width, height]).padding(5).round(true)(
        secondTreemapRoot
      );
      ...
      const sortedLeaves = secondTreemapRoot
        .leaves()
        .sort((a, b) => b.value - a.value);
      ...
      secondTreemap
        ...
        .on('mouseover', function (event, d) {
          if ((stateObject.treemapLevel === 1) & (d.data.name !== undefined)) {
            d3.select(this).attr('stroke', '#ac513b').attr('stroke-width', 4);
            ...
            d3
              ...
              .style('box-shadow', '0px 5px 5px rgba(0, 0, 0, 0.3)') // Add drop shadow
              .style('left', `${x + 120}px`)
              .style('top', `${y + 320}px`).html(`Depth Range: 
    ...
    async function createZoomedFish(node) {
      try {
        const fishData = await d3.json('./data/updated_final_copy.json');
        ...
        const fishContainer = d3
          ...
          .style(
            'top',
            `${
              parseFloat(d3.select('svg').attr('viewBox').split(' ')[1]) + 100
            }px`
          )
          .style(
            'left',
            `${
              parseFloat(d3.select('svg').attr('viewBox').split(' ')[0]) + 220
            }px`
          )
          .style('width', `${d3.select('svg').attr('viewBox').split(' ')[2]}px`)
          .style(
            'height',
            `${d3.select('svg').attr('viewBox').split(' ')[3]}px`
          )
          .style('pointer-events', 'none')
          .style('z-index', 1);
        ...
        for (let i = 0; i < 20; i++) {
          const randomFish =
            mergedData[Math.floor(Math.random() * mergedData.length)];
          ...
          const fish = fishContainer
            ...
            .style('position', 'absolute')
            .style('width', `${Math.random() * 50 + 60}px`)
            .style('height', 'auto')
            .style('top', `${Math.random() * 80 + 35}%`)
              ...
              d3
                ...
                .style('top', `${y + 20}px`).html(`
                  <a href="${d.record_link}" target="_blank">
                    <img src="${d.thumbnail}" alt="Fish Thumbnail" style="width: 100px; height: auto; border-radius: 5px;">
                  </a>
        ...
        console.error('createZoomedFish 함수 오류:', error);
      }
    }
    ...
    d3.select('body').on('click', function (event) {
      if (!event.target.closest('svg')) {
        ...
        d3.selectAll(
          '.legend-group, tooltip, .tooltip-depth, .tooltip-size, .tooltip-dep'
        )
          .transition()
          .duration(400)
          .style('opacity', 0)
